...
const IMERender = (function() {
  ...
  var toggleCandidatePanel = function(expand) {
    ...
    var unitWidth = Math.floor(document.getElementById('keyboard').clientWidth /
                                                      numberOfCandidatesPerRow);
    ...
    if (expand) {
      ...
      candidatePanelToggleButton.style.width = (unitWidth - 8) + 'px';
    } else {
      IMERender.ime.classList.remove('full-candidate-panel');
      IMERender.ime.classList.add('candidate-panel');
      candidatePanelToggleButton.style.width = unitWidth + 'px';
    }
  };
  ...
  var showCandidates = function(candidates, noWindowHeightUpdate) {
    ...
    if (candidatePanel) {
      ...
      if (inputMethodName == 'latin') {
        ...
        docFragment = _layoutCandidates(candidates, 1, true);
      }
  ...
  var showMoreCandidates = function(candidates, rowLimit) {
    if (! rowLimit) rowLimit = -1;
    document.getElementById('keyboard-candidate-panel').appendChild(
      _layoutCandidates(candidates, rowLimit)
    );
  };
  ...
  var _layoutCandidates = function(candidates, rowLimit, indentFirstRow) {
    var candidatePanel = document.getElementById('keyboard-candidate-panel');
    var candidatePanelToggleButton =
      document.getElementById('keyboard-candidate-panel-toggle-button');
    ...
    var docFragment = document.createDocumentFragment();
    if (candidates.length == 0) return docFragment;

    var rowDiv = document.createElement('div');
    rowDiv.classList.add('candidate-row');
    if (indentFirstRow) rowDiv.classList.add('candidate-row-first');

    var nowUnit = 0;
    var rowCount = 0;

    if (rowLimit < 0) rowLimit = 999;
    ...
    for (var i = 0; i < candidatesLength; i++) {
      ...
      span.dataset.selection = true;
      span.dataset.data = data;
      span.style.width = (unit * unitWidth - 2) + 'px';
    ...
    if (layout) {
      ...
      [].forEach.call(
        ...
        function(item) {
          ...
          item.style.width = (unit * unitWidth - 2) + 'px';
        }
      );
      ...
      if (candidatePanelToggleButton) {
        if (keyboard.classList.contains('full-candidate-panel')) {
          candidatePanelToggleButton.style.width = (unitWidth - 8) + 'px';
        } else {
          candidatePanelToggleButton.style.width = unitWidth + 'px';
        }
