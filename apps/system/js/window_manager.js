...
var WindowManager = (function() {
  ...
  windows.addEventListener('transitionend', function frameTransitionend(evt) {
    ...
    if (screenElement.classList.contains('switch-app')) {
      if (classList.contains('closing')) {
        ...
        // XXX: without this setTimeout()
        // there will be no opening transition.
        // could be https://bugzilla.mozilla.org/show_bug.cgi?id=780692
  ...
  // We centralize all that here but not all of them applys.
  windows.addEventListener('mozbrowserfirstpaint', function firstpaint(evt) {
    ...
    setTimeout(function firstpainted() {
      // Save the screenshot
      // Remove the background only utill we actually got the screenshot,
