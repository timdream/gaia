...
suite('system/AppWindowManager', function() {
  ...
  suite('Hierarchy functions', function() {
    ...
    suite('isActive', function() {
      test('TaskManager is active', function() {
        this.sinon.stub(taskManager, 'isActive').returns(true);
        assert.isFalse(appWindowManager.isActive());
      });

      test('TaskManager is not active and current app is active', function() {
        this.sinon.stub(taskManager, 'isActive').returns(false);
        appWindowManager._activeApp = app1;
        this.sinon.stub(app1, 'isActive').returns(true);
        assert.isTrue(appWindowManager.isActive());
      });

      test('TaskManager is inactive and current app is inactive', function() {
        this.sinon.stub(taskManager, 'isActive').returns(false);
        appWindowManager._activeApp = app1;
